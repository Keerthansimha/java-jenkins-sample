pipeline {
    ...
    environment {
        REMOTE_SERVER = '34.230.89.201'    // IP or hostname of the target server
        ...
        stage('Checkout') {
            steps {
                checkout([$class: 'GitSCM', 
                    ...
                    extensions: [], 
                    userRemoteConfigs: [[credentialsId: 'github-access', url: 'https://github.com/Keerthansimha/java-jenkins-sample.git']]
                ])
            }
        }
            ...
            steps {
                sshagent(credentials: ['ssh-agent']) {
                    script {
                        ...
                        if (isUnix()) {
                            sh """
                               scp  -i /home/ubuntu/.ssh/id_ed25519 /var/lib/jenkins/workspace/java_pipeline/target/jb-hello-world-maven-0.2.0.jar  ubuntu@ec2-34-230-89-201.compute-1.amazonaws.com:/var/www/html/myapp
                               ssh  -i home/ubuntu/.ssh/id_ed25519  ubuntu@ec2-34-230-89-201.compute-1.amazonaws.com
                            """
                        } else {
                            bat """
