pipeline {
    ...
    environment {
        REMOTE_SERVER = '18.212.96.230'    // IP or hostname of the target server
            ...
            steps {
                sshagent(credentials: ['ssh-agent']) {
                    script {
                        ...
                        if (isUnix()) {
                            sh """
                               scp  -i /home/ubuntu/.ssh/id_ed25519 /var/lib/jenkins/workspace/java_pipeline/target/jb-hello-world-maven-0.2.0.jar   ubuntu@ec2-18-212-96-230.compute-1.amazonaws.com:/var/www/html/myapp
                               ssh  -i home/ubuntu/.ssh/id_ed25519  ubuntu@ec2-18-212-96-230.compute-1.amazonaws.com
                            """
                        } else {
                            bat """
