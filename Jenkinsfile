pipeline {
    ...
    environment {
        REMOTE_SERVER = '3.88.138.142'    // IP or hostname of the target server
            ...
            steps {
                sshagent(credentials: ['ssh-agent']) {
                    script {
                        ...
                        if (isUnix()) {
                            sh """
                               scp -i /home/ubuntu/.ssh/id_ed25519 target/jb-hello-world-maven-0.2.0.jar ubuntu@ec2-3-88-138-142.compute-1.amazonaws.com:/var/www/html/myapp
                               ssh -i /home/ubuntu/.ssh/id_ed25519  ubuntu@ec2-3-88-138-142.compute-1.amazonaws.com'cd /var/www/html/myapp && java -jar jb-hello-world-maven-0.2.0.jar'
                            """
                        } else {
                            bat """
