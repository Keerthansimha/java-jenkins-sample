pipeline {
    ...
    environment {
        REMOTE_SERVER = '54.242.63.237'    // IP or hostname of the target server
        REMOTE_USER = 'ubuntu'             // SSH username for the target server
        REMOTE_PATH = '/var/www/html/' // The path where you want to deploy
            ...
            steps {
                sshagent(credentials: ['ssh-agent']) {
                    script {
                        ...
                        if (isUnix()) {
                            sh """
                               scp -i /home/ubuntu/.ssh/id_ed25519 target/jb-hello-world-maven-0.2.0.jar ubuntu@ec2-54-242-63-237.compute-1.amazonaws.com:/var/www/html/myapp
                               ssh -i /home/ubuntu/.ssh/id_ed25519  ubuntu@ec2-54-242-63-237.compute-1.amazonaws.com'cd /var/www/html/myapp && java -jar jb-hello-world-maven-0.2.0.jar'
                            """
                        } else {
                            bat """
