            ...
            steps {
                sshagent(credentials: ['ssh-agent']) {
                    script {
                        ...
                        if (isUnix()) {
                            sh """
                               scp -o StrictHostKeyChecking=no -i /home/ubuntu/.ssh/id_ed25519 /var/lib/jenkins/workspace/java_pipeline/target/jb-hello-world-maven-0.2.0.jar  ubuntu@ec2-54-227-91-72.compute-1.amazonaws.com:/var/www/html/myapp
                               ssh  -o StrictHostKeyChecking=no -i home/ubuntu/.ssh/id_ed25519 ubuntu@ec2-54-227-91-72.compute-1.amazonaws.com 
                            """
                        } else {
                            bat """
